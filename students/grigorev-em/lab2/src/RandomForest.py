...
class RFClassifier:
    def __init__(self, n_estimators: int = 100, max_features = "sqrt", max_depth: int | None = None, min_samples_split: int = 2, random_state: int | None = 42):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        for _ in range(self.n_estimators):
            boot_idx = self.generator.randint(0, n_samples, size=n_samples)
            ...
            tree = DecisionTreeClassifier(
                ...
                random_state=self.generator.randint(0, 2**31),
            )
        ...
        voted = oob_counts > 0
        if voted.sum() > 0:
            oob_preds = np.argmax(oob_votes[voted], axis=1)
    ...
    def _compute_feature_importances(self, X: np.ndarray, y: np.ndarray):
        ...
        for tree, oob_mask in zip(self.estimators_, self._oob_masks):
            ...
            for j in range(n_features):
                X_perm = X_oob.copy()
                X_perm[:, j] = self.generator.permutation(X_perm[:, j])
