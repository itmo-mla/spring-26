...
def plot_learning_curve(n_estimators_list, train_scores, test_scores, oob_scores=None, img_name="learning_curve.png"):
    """
    Plot learning curve: accuracy vs number of estimators.
    ...
    train_scores : list of float
        Training accuracy for each n_estimators.
    test_scores : list of float
        Test accuracy for each n_estimators.
    oob_scores : list of float or None
        OOB accuracy for each n_estimators.
    img_name : str
        Filename to save plot.
    """
    plt.figure(figsize=(10, 6))
    plt.plot(n_estimators_list, train_scores, 'o-', label="Training score")
    plt.plot(n_estimators_list, test_scores, 's-', label="Test score")
    if oob_scores is not None:
        plt.plot(n_estimators_list, oob_scores, '^-', label="OOB score")
